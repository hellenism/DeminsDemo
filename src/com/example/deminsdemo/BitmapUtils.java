...
public class BitmapUtils {
	 ...
	 */
	public static Point getBitmapWidthAndHeight(Context context, int resId) {
		BitmapFactory.Options options = new BitmapFactory.Options();
		// 先解码图片的边框，而非整张图片，从而可以得到图片的宽高
		options.inJustDecodeBounds = true;
	 ...
	 */
	public static Point getBitmapWidthHeightByReferentSzie(Context context,
			int resId, float compareWith, float compareHeight) {
		Point resultSize = new Point();
		Point targetImageSize = getBitmapWidthAndHeight(context, resId);
...
//		int screenHeight = res.getDisplayMetrics().heightPixels;
		...
		int screenHeight = DeviceUtils.getAvailableScreenHeightPixels(context);
